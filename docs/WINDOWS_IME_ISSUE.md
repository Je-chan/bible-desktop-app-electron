...
Google Sheets, Excel Online, VS Code (Monaco Editor) 등 웹 기반의 복잡한 입력기를 다루는 앱들이 사용하는 **검증된 패턴**입니다.
...
│   시각적 표시:                                                   │
│   ┌─────────┐  ┌─────────┐  ┌─────────┐                        │
│   │ 책: 창  │  │ 장: 1   │  │ 절: 1   │  ← div (플레이스홀더)  │
│   └─────────┘  └─────────┘  └─────────┘                        │
│        ▲                                                        │
│        │                                                        │
│   ┌─────────┐                                                   │
│   │ [input] │  ← 실제 input (absolute positioning으로 오버레이) │
│   └─────────┘                                                   │
│                                                                 │
│   Tab 키 입력 시:                                                │
│   - input은 그대로 DOM에 유지 (절대 언마운트 안 됨)              │
│   - CSS position만 변경하여 다음 필드 위치로 이동                │
│   - Focus는 같은 DOM 요소에 유지 → Blur 없음 → IME 유지!        │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
export const Footer = ({ book, chapter, verse, masterInputRef, onSearch, ... }) => {
  ...
  // 현재 활성화된 필드의 값
  const currentValue = activeField === 'book' ? book
                     : activeField === 'chapter' ? chapter
                     : verse
...
| 파일 | 역할 |
|------|------|
| `src/renderer/src/widgets/Footer/Footer.tsx` | Master Input 패턴 구현 |
| `src/renderer/src/features/verse-search/useVerseSearch.ts` | 검색 로직 (handleSearch) |
| `src/renderer/src/App.tsx` | masterInputRef 관리, Footer 연동 |
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  문제: Windows에서 Input 간 focus 이동 시 IME 리셋              │
│                                                                 │
│  원인: Chromium이 blur 시 IME context를 해제하고,               │
│        focus 시 기본(영문) context를 연결함                     │
│                                                                 │
│  해결: Master Input 패턴                                        │
│        - 단일 input을 항상 DOM에 유지                           │
│        - Tab 키의 기본 동작을 막고 (e.preventDefault)           │
│        - CSS로 위치만 변경하여 필드 간 이동 시뮬레이션          │
│        - Focus가 이동하지 않으므로 blur 없음 → IME 유지         │
│                                                                 │
│  결과: Tab으로 책 → 장 → 절 이동 시에도 IME 100% 유지 ✅        │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
